...
public class StackOverFlowSurvey {
    ...
    public static void main(String[] args) throws Exception {
        ...
        */
        ...
        SparkSession spark = SparkSession.builder().appName("StackOverFlowSurvey").master("local").getOrCreate();

        Dataset<Row> responses = spark.read().option("header", "true").csv("in/2016-stack-overflow-survey-responses.csv");
        responses.printSchema();
        Dataset<Row> responsesFromAfganisthan = responses.filter(col("country").$eq$eq$eq("Afghanistan"));
        //Dataset<Row> responsesFromAfganisthan = responses.filter(col("country").equalTo("Afghanistan"));
        responsesFromAfganisthan.show(5);

        Dataset<Row> occupationCounts = responses.groupBy(col("occupation")).count();
        occupationCounts.show(5);

        Dataset<Row> salaryAndAgeAsInts = responses
                .withColumn("salary_mid_point", col("salary_midpoint").cast("integer"))
                .withColumn("age_mid_point", col("age_midpoint").cast("integer"));
        salaryAndAgeAsInts.printSchema();
        salaryAndAgeAsInts.show(5);

        Dataset<Row> ageLessThan20 = responses.filter(col("age_midpoint").$less(20));
        ageLessThan20.show(5);

        responses.orderBy(col("salary_midpoint").desc()).show(5);

        Dataset<Row> salaryMiddlePoint = salaryAndAgeAsInts.withColumn(
          "salary_bucket", col("salary_midpoint").divide(2000).cast("integer").multiply(2000)
        );
        salaryMiddlePoint.show(5);

        salaryMiddlePoint.groupBy("salary_bucket").count().orderBy("count").show();
    }
}
